...
contract FROSTCounterOptimized {
    ...
    function setNumber(uint128 newNumber, uint256 signatureRX, uint256 signatureRY, uint256 signatureZ) public {
        uint256 memPtr = Memory.allocate(0x60);
        Memory.writeWord(memPtr, 0x00, uint256(uint160(address(this))));
        Memory.writeWord(memPtr, 0x20, uint256(nonce));
        unchecked {
            nonce++;
        }
        Memory.writeWord(memPtr, 0x40, newNumber);

        bytes32 messageHash = bytes32(Hashes.efficientKeccak256(memPtr, 0x0c, 0x54));
